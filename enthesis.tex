...
\tiivistelma{%
  ...
  Sen tulee kertoa täsmälleen samat asiat kuin englanninkielinen
  abstrakti.
}
\abstract{%
...
% is more than one
 ...
 % you don't need this line in a master's or bachelor's thesis
\type{Template and manual for a thesis document class}

% change this line to match your study subject
...
don't have prefaces, but if you write one, keep it short (at most one
page).
...
Master's thesis in the Faculty of Information Technology
(mathematical information technology) at the University of Jyväskylä.
You can send comments, suggestions for improvement and bug reports to
...
You may well need to use mathematical notation in your thesis. Unless you have
a reason otherwise, it is good practice to use features from different packages
in preference-order. Prefer most the high level features from the
\texttt{amsmath} package \parencite[see][]{amsmath-manual}, then from \LaTeX
\parencite[see][]{lamport94:_latex}, and then from \TeX itself
\parencite[see][]{knuth86:_texbook}. The aforementioned manuals are in English,
but \fullcite{kaijanaho03:_latex_ams_latex} gives an overview of these topics
in Finnish. As a demonstration, here is a derivation of the quadratic formula
by completing the square using the \texttt{align*} environment from the
\texttt{amsmath} package:

\begin{align*}
  ax^2 + bx + c &= 0 && \text{We start from a general quadratic equation} \\
  x^2 + \frac{b}{a}x + \frac{b}{2a}^2 &= -\frac{c}{a} + \frac{b}{2a}^2 && \text{Algebric rearrangement} \\
  \left(x + \frac{b}{2a}\right)^2 &= \frac{b^2 - 4ac}{4a^2} && \text{Complete the square} \\
  x &= \frac{-b \pm \sqrt{b^2 - 4ac}}{2a} && \text{Solve for x}
\end{align*}
...
Windows, use the Mik\TeX\ package biber-windows-x64.
...
Some installations have a bug in their system libraries that make
surnames beginning with W be sorted incorrectly at the beginning of
the bibliography in Finnish mode.  This has been observed in
Spring~2019 at least in Overleaf.  Currently no other solution is
known than trying different installations.  This bug will most likely
be fixed eventually.
...
\begin{table}[ht]\centering
  \begin{tabular}{lp{9cm}}
    ...
    & The contact information of the author (optional) \\
    {\string\supervisor}
...
\item The standard option ``twoside'' works as well.  It can be used,
  for exampkle, in preparing an unofficial version for your own
  bookshelf.  It should not be used in preparing the graded and
  archived version.
